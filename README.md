...
🔗 Project URL: https://github.com/bluemoney/XSDBasedXMLEditor
...
直接使用瀏覽器 (Chrome, Firefox, Edge, Safari) 開啟 index.html 即可使用。

使用方式 (Usage)
載入 XSD：點擊左上角的 「📁 載入 XSD 檔案」，選擇您的 .xsd 架構檔。
...
查看資訊：右側面板會顯示當前節點的 XSD 定義、XPath 與限制條件。

驗證：點擊 「✓ 驗證 XML」 檢查文件是否符合規範。

匯出：點擊 「💾 匯出 XML」 下載編輯完成的檔案。
...
📄 授權 (License)
本專案採用 Apache-2.0 授權條款。詳細內容請參閱 LICENSE 檔案。

Copyright (c) 2024 bluemoney.
